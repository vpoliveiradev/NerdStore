...
namespace NerdStore.Catalogo.Domain.Tests
{
    public class ProdutoTests
    {
        ...
        public void Produto_Validar_ValidacoesDevemRetornarExceptions()
        {
            ...
            Assert.Equal("O campo Nome do produto não pode estar vázio", ex.Message);

            ex = Assert.Throws<DomainException>(() =>
                new Produto("Nome", string.Empty, false, 100, Guid.NewGuid(), DateTime.Now, "Imagem", new Dimensoes(1, 1, 1))
            );

            Assert.Equal("O campo Descricao do produto não pode estar vázio", ex.Message);
            ...
            Assert.Equal("O campo CategoriaId do produto não pode estar vázio", ex.Message);

            ex = Assert.Throws<DomainException>(() =>
                new Produto("Nome", "Descricao", false, 100, Guid.NewGuid(), DateTime.Now, string.Empty, new Dimensoes(1, 1, 1))
            );

            Assert.Equal("O campo Imagem do produto não pode estar vázio", ex.Message);
